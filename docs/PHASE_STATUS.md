...
**فاز ۲.۵ — گام ۳: AI API endpoints — تکمیل‌شده:**
- دو اندپوینت فقط-خواندنی طبق قرارداد دقیق §3 طرح:
  - **`GET /kpis/{id}/insight`** (`app/routers/kpis.py`): analyst+ (viewer → 403؛ unauth → 401)؛ cross-org → 404؛ KPI disabled → 400؛ منبع pending → 400 (همان سمانتیک compute/series — بدون تکرار قاعده)؛ پارامتر `window_days` اختیاری با clamp ۷..۳۶۵ (پیش‌فرض ۲۸)؛ پنجره تعریف KPI که دو پنجره کامل مجاور نمی‌پذیرد → 422 (§8.3)
  - **`GET /assistant/briefing`** (`app/routers/assistant.py` جدید): analyst+؛ KPIهای enabled سازمان با ترتیب deterministic `GET /kpis` (created_at desc, name asc)؛ سقف bounded `MAX_BRIEFING_KPIS = 20`؛ KPI با منبع map نشده یا پنجره نامعتبر حذف می‌شود (خطا نیست — empty org رفتار valid §3 است)؛ `flagged_count` = تعداد packetهای پرچم‌دار
- ماژول جدید `app/services/insight_packets.py` (لایه چسبان گام ۳ — راسترها لاغر می‌مانند): `clamp_window_days` / `effective_windows` (دو پنجره برابر مجاور با intersect پنجره تعریف KPI؛ فضای ناکافی → `WindowError`) / `build_packet_for_windows` (تنها نقطه دسترسی DB — فقط ORM با فیلتر صریح organization_id + data_source_id، لایه ۳ دفاع کنار RLS) / `narrate_with_factory` (حداکثر یک فراخوانی provider per request؛ factory None → `narrative: null` + `narrative_error="provider_unavailable"`؛ خطای typed provider → کد sanitized — هرگز 500، هرگز کلید/بدنه خام/DSN)
- اسکیماهای جدید `app/schemas/assistant.py`: `KpiInsightOut` / `BriefingKpiOut` / `BriefingOut` / `NarrativeOut` / `MoverOut` — همه مقادیر عددی decimal-string چهاررقم (هرگز float — قرارداد فاز ۲.۳)؛ هیچ organization_id/credential در هیچ پاسخی نیست؛ اعداد فقط از موتور pure گام ۱ می‌آیند (هیچ قاعده تجمیع/آستانه‌ای اینجا تکرار نمی‌شود)
- موتور pure گام ۱ (`app/services/insights.py`)، موتور KPI فاز ۲.۳ و Context Catalog فاز ۲.۴ دست‌نخورده‌اند؛ بدون migration جدید (گام ۳ هیچ تغییر schema ندارد)؛ بدون SQL خام در لایه جدید (اسکن ایستا)
- تست‌ها: ۲۳ integration روی PostgreSQL واقعی (`tests/test_assistant_api.py` — الگوی established: skip ماژول-سطح بدون DB، seed در fixture با engine superuser، cleanup فقط ردیف‌های خود ماژول با پیشوندهای ins-/اسلاگ‌های org-*-ins): ماتریس نقش‌ها (analyst/owner ✓، viewer 403، unauth 401/403)؛ cross-org → 404؛ disabled → 400؛ منبع pending → 400؛ window پیش‌فرض ۲۸ با packet دستی-محاسبه‌شده (current=150/previous=90/change=60/change_pct=0.6667)؛ clamp پایین ۷ و بالا ۳۶۵؛ پنجره تعریف تنگ → 422؛ خالی sum → 0.0000/flat و خالی avg → unknown (قرارداد §3.3/§2.1 — بدون عدد ساختگی)؛ دقت Decimal (هرگز float در wire)؛ movers مرتب نزولی |contribution|؛ determinism دو-فراخوانی؛ بریفینگ: empty org → 200 خالی، ترتیب created_at desc، حذف disabled/منبع pending/پنجره تنگ، clamp ۷، narrative null بدون AI، stub provider با seam ماژول-سطح factory → narrative پر + اثبات non-disclosure در packet (بدون password/host/DSN/slug org دیگر)؛ LLM واقعی هرگز صدا زده نمی‌شود
- گیت: **۲۷۹ passed، 0 failed، 0 skipped** در یک session روی PostgreSQL واقعی (۲۵۶ baseline + ۲۳ جدید — بدون رگرسیون)؛ `alembic current` = `0006_kpi_definitions` تک‌head؛ `alembic check` = «No new upgrade operations detected»؛ RLS دست‌نخورده؛ role اپ NOSUPERUSER/NOBYPASSRLS
- گام بعدی فاز ۲.۵ (شروع نشده): گام ۴ گیت نهایی
